...
def drop_end_point_voxels( hits       : pd.DataFrame
                         ...
                         , min_voxels : int
                         , e_type     : HitEnergy
                         , contiguity : Contiguity) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    while modified:
        modified = False
        tracks = make_track_graphs(voxels, vox_size, contiguity)
        ...
        for i, t in enumerate(tracks):
            ...
            for i_extreme in find_extrema(t)[:-1]: # last one is a distance
                ...
                if extreme.E < threshold:
                    ...
                    ### beyond itself
                    n_neighbours = sum(neighbours(extreme, v, vox_size, contiguity) for _, v in voxels.iterrows())
                    if n_neighbours > 1:
