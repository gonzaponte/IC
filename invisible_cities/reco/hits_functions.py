...
def merge_NN_hits( hits      : pd.DataFrame
    ...
    """
        Redistribute the energy of NN hits to the closest hits. The
        amount added is proportional to the receiving hit's energy.
        The parameter `same_peak` controls whether the energy is
        shared among hits within the same peak or other peaks can be
        considered.
        If all the hits are NN, an empty dataframe is returned.
    """
    are_nn  = hits.Q == NN
...
def threshold_hits( hits         : pd.DataFrame
    ...
    """
        Apply a charge threshold to the hits.
        The energy of the hits below the threshold is redistributed
        among the hits in the same time/z slice.
        The parameter `on_corrected` controls whether `Q` or `Qc`
        is used for the threshold.
    """
    def process_slice(hits):
        ...
        out.loc[:, "E" ] = out.E .sum() * weights
        out.loc[:, "Ec"] = out.Ec.sum() * weights
        return out
