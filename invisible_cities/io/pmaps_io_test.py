...
def test_store_peak_s1(output_tmpdir, KrMC_pmaps):
...
def test_store_peak_s2(output_tmpdir, KrMC_pmaps):
...
def test_store_pmap(output_tmpdir, KrMC_pmaps):
...
def test_load_pmaps_as_df(KrMC_pmaps):
    filename, true_dfs, *_ = KrMC_pmaps
    read_dfs = pmpio.load_pmaps_as_df(filename)
    for read_df, true_df in zip(read_dfs, true_dfs):
        assert_dataframes_equal(read_df, true_df)


def test_load_pmaps_as_df_without_ipmt(KrMC_pmaps_without_ipmt):
    filename, true_dfs, *_ = KrMC_pmaps_without_ipmt
    read_dfs = pmpio.load_pmaps_as_df(filename)
...
def test_load_pmaps(KrMC_pmaps):
    filename, *_, true_pmaps = KrMC_pmaps
...
def test_build_pmt_responses(KrMC_pmaps, signal_type):
    if   signal_type is S1:   df,  _, _, pmt_df,      _ = KrMC_pmaps[1]
    elif signal_type is S2:    _, df, _,      _, pmt_df = KrMC_pmaps[1]
...
def test_build_sipm_responses(KrMC_pmaps):
    _, _, df_event, _, _ = KrMC_pmaps[1]
