...
@given(float_arrays(min_value=-10, max_value=10))
...
@given(random_length_float_arrays(min_length = 1, max_length = 10))
...
@given(floats(min_value = -20., max_value = -10.),
       floats(min_value = -20., max_value = -10.))
def test_expo_double_negative_sign(x, mean):
    assert fitf.expo(x, 1., mean) > fitf.expo(x+1., 1., mean)


@given(floats(min_value = +10., max_value = +20.),
...
@given(integers(min_value = -10, max_value = +10),
       ...
       integers(min_value = -10, max_value = +10).filter(lambda x: x != 0))
...
@given(floats(min_value = -10., max_value = +10.),
       floats(min_value =   0., max_value = +10.).filter(lambda x: x != 0.))
...
@mark.parametrize("     func            known_pars".split(),
...
@mark.parametrize("func", (fitf.profileX, fitf.profileY))
def test_empty_dataset_yields_nans(func):
    N = 50
    empty = np.array([])
