...
def get_pmaps_dict_and_event_numbers(filename):
...
def KrMC_pmaps(ICDATADIR):
    ...
    dict_pmaps, evt_numbers = get_pmaps_dict_and_event_numbers(test_file)
    s1df, s2df, sidf, s1pmtdf, s2pmtdf = load_pmaps_as_df(test_file)
...
@pytest.fixture(scope='session')
def KrMC_pmaps_without_ipmt(ICDATADIR):
